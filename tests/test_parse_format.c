...
static void test_parse_golden(void) {
    ksuid_t id;
...
static void test_parse_size_errors(void) {
...
static void test_parse_value_errors(void) {
    ksuid_t id;
    /* Mirrors upstream TestIssue25 (ksuid_test.go:100-111). */
    ASSERT_EQ_INT(
        ksuid_parse(&id, "aaaaaaaaaaaaaaaaaaaaaaaaaaa", KSUID_STRING_LEN),
        KSUID_ERR_STR_VALUE);
    ASSERT_EQ_INT(
        ksuid_parse(&id, "aWgEPTl1tmebfsQzFP4bxwgy80!", KSUID_STRING_LEN),
        KSUID_ERR_STR_VALUE);
    /* One past Max -- value overflow rather than alphabet violation. */
    ASSERT_EQ_INT(
        ksuid_parse(&id, "aWgEPTl1tmebfsQzFP4bxwgy80W", KSUID_STRING_LEN),
        KSUID_ERR_STR_VALUE);
}

static void test_parse_or_nil(void) {
